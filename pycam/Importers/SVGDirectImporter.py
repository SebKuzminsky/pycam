...
class SVGXMLParser:
    ...
    def _append_svg_path(self, path):
        if not self.groups:
...
def _get_polygons_from_svg_path(path, z, interpolation_accuracy=0.1,
    ...
    for segment in path:
        ...
        if isinstance(segment, svg.path.Line):
            ...
            step_count = int(math.ceil(segment.length() / interpolation_accuracy))
...
def import_model(filename, callback=None, **kwargs):
    if isinstance(filename, pycam.Utils.URIHandler):
        if not filename.is_local():
            raise GenericError("SVGDirectImporter: remote SVG model locations are not supported. "
                               "Please download the file first and open it locally.")
        filename = filename.get_local_path()
